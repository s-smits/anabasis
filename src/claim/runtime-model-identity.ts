...
function inspectPiIdentity(
  ...
  return {
    ...
    // 93d2fc3d7 stopped substituting the pinned model for the Codex route's absent attestation,
    // and both Astra claims of 2026-09-08 were refused on that alone. The absence is a fact
    // about the route: the identity stays unverified without refusing the claim. Any other
    // provider's null model, and any different reported model, still refuses.
    unattested:
      identity.provider.model === null && identity.provider.id === UNATTESTING_PROVIDER
...
function inspectCase(
  ...
  // Read before any branch returns, including the two counts below. Each refusal here records
  // the case as `blocking`, and `blocking` says only that the pin was not evidenced — an absent,
  // blank or internally inconsistent census. An identity naming another served model says
  // something else entirely: that a different condition was measured. The two clauses route to
  // different owners, one of them an environment clause, so a contradiction recorded beside a
  // broken count must not leave through the count's exit. It sat under the count checks until
  // 2026-09-20, where a census whose turn totals disagreed took its crossed identities with it:
  // the reading a wrong-model run needs is exactly the one a damaged row is likeliest to carry.
  // Nothing here reads a count, so the order costs one map over rows a refusal would discard.
  ...
  if (row.completedTurns === 0) {
    ...
    // independent readers filed these IDs against the run's typed non-results.
    //
    ...
    // round of that run with failing cases. `unattested` is the bucket for this — it leaves
    // `modelIdentity` unverified in the statement and refuses nothing — and it is the line
    // `src/truth/runtime-blocker.ts` already draws on the same count, where zero completed turns
    // is an environment blocker only with no tool calls and no accepted submit.
    //
    // An unverified case keeps the refusal: no bytes were graded, so there is nothing the missing
    // attestation would have been the identity of.
...
export function runtimeIdentityFindings(
  ...
  // Each scored case with the word `inspectCase` asks for, so the loop below reads the state off
  // the row it is already holding.
