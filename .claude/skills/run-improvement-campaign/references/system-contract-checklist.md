...
current angle 21"; the ledger producer/consumer join flips CL-F in deterministic session 30.

| ID | Ready at reviewed source? | Required witness | Owner |
|---|---|---|---|
| CL-A | true | recorded one-line difficulty note per frozen climb/descend-authored task set | current angle 10 |
| CL-C | true | history-to-prompt binding (climb readout + last-two-battery projection reaches the successor authoring prompt) | current angle 17 |
| CL-E | true | comparable on-note consecutive task sets suppliable to current angle 21 (public task sets, notes, task-set hashes) | current angle 21 |
| CL-F | true | difficulty-author readout rows and their live consumer (`readClimbReadout`, `climbReadout`; before 2026-09-21 `readClimbBatteries`, `climbLedgerRows`). On a legacy tree that still emits a per-run `climb-economy.json`, its captured digest is reconciled too; current trees have no second economy view. | deterministic session 30 |

Reviewed source revision for the four `true` rows: published stack successor #361 at
...
and `src/run/climb-readout-frame.ts` replace the note writers); current WRI supplies angle 21's public
comparison. The product-side CL-A, CL-C and CL-F witnesses were rechecked in the separate clean
...
preparation record. `Run angles` names the current whole-run review row that can inspect the
corresponding run evidence; `—` means it is not a run-evidence decision. Current deterministic
rows are `A–H`; current semantic rows are angles `1–24`. For archive backtracking, the definitive
legacy map is `A=old1…H=old8`, `current angle 1=old9` through `21=old29`, `22=old31`,
`23=old32`, `24=old33`. Old 30 has no current angle; deterministic session 30 is a separate
CL-F reconciliation and is not a renumbered semantic session.

| Legacy ID | 2026-08-09 finding | Disposition | Current home | Exact correction or limit | Run angles |
|---|---|---|---|---|---|
...
| B4 | partly correct | split | fixed experiment; rebuild event; climb event | Experiment kind, diff-derived owner and task-condition movement are separate. A build or climb need not have a routed owner. | current angles 9, 10 |
| B5 | unobservable | procedure-only | prediction lineage note; P3; X3 | Product evidence still writes no joined frozen-prediction lineage. Preserve it append-only as operator evidence. | current angles 13, 17 |
| B6 | unobservable | procedure-only | P1; P2 | Trigger, effect, falsifier and deciding evidence are useful, but their freeze is an operator digest rather than a run fact. | current angles 13, 17 |
...
| B8 | partly correct | split | fixed source/runtime identity; preparation note | Deterministic runtime, source, provider and lock checks are real. Freshness, launch authority and paid count are not one durable no-turn result. | A, current angle 13 |
...
| C6 | partly correct | split | C3c; C6 | Static corpus validity, executed discrimination and census verdict are separate. `census.json` has no per-control execution rows or direct task-set hash. | E, current angles 5, 6 |
| C7 | ambiguous | split | C3d; C6; C7 | Static verifier wiring, live control discrimination and F2 solvability settle independently. The host verifier/verifier alone owns truth. | F, current angles 5, 6, 7 |
...
| C11 | correct only when triggered | merge | C2; rebuild event | A later rebuild must join predecessor child to later base. No rebuild is `N/A`. | C, current angle 9 |
...
| C13 | correct only when triggered | merge | C2; deletion event | Continuity and deletion are one committed-tree check; deletion also requires absence from the child tree. | C, current angle 9 |
| C14 | partly correct | split | C5; M1; worker-close evidence | Fingerprint owns three hashes. Source, runtime, isolation, policy and worker-close facts live in other evidence and must not be attributed to it. | E, H, current angle 14 |
| M1 | correct only when triggered | keep | C7 | F2 exists only after the gate is reached; an earlier block is pending, not failed. | F |
| M2 | partly correct | split | C6; C8 | Control census and adoption have different owners and no combined verdict. | E, current angle 11 |
...
| M4 | partly correct | keep | M2; M3 | Normal producers form the four-way partition, but the current JSON boundary accepts any string as a non-result kind unless the caller checks the shared closed set. | current angle 1 |
| M5 | correct only when triggered | keep | M4 | Judge 1 has typed valid/invalid/incomplete/off/unavailable states; only complete control-valid evidence can pass. | current angle 2 |
| M6 | obsolete | obsolete | — | The component was the Repair Engineer, removed on 2026-09-04 with the repair experiment. Its replacement, the deterministic rebuild advice packet, is derived from recorded case rows and has no diagnosis-coverage denominator to record. | current angle 3 |
| M7 | partly correct | split | M6; M7; C8; promotion event; X1 | Claim/refusal, readiness, adoption, promotion and terminal each have an independent trigger and owner. | current angles 11, 13 |
| M8 | partly correct | split | rebuild event; climb event | A rebuild reopens the harness and is measured as a build; climb freezes the harness and changes only the task difficulty level. `census.json` does not directly join the task-set identity. | current angles 9, 10 |
| M9 | partly correct | keep | M2; M3; typed non-result event | Host case evidence owns truth and denominators. Reject an unknown non-result kind at review because the JSON parser alone does not. | current angle 1 |
| X1 | partly correct | split | X1; X2 | Typed closure and cross-projection identity agreement are separate. Snapshot command completion does not prove semantic schema/digest agreement. | A, current angle 13 |
| X2 | unobservable | procedure-only | P1-P3; X3 | Frozen prediction resolution remains operator-maintained; current run evidence has no complete lineage join. | current angles 13, 17 |
| X3 | partly correct | split | X2; operator learning note | The scorecard is a diagnostic projection. Skill learning is prose and cannot be a product pass. | A, current angle 13 |
| X4 | unobservable | procedure-only | X4 | Git diff is observable; positive/hostile commands, gate result and checkpoint are operator receipts. | current angle 13 |
| X5 | partly correct | procedure-only | X5 | A zip review is advisory and has no product authority. It is `N/A` when authoring never reaches the required terminal/commit boundary. | current angle 13 |
| X6 | partly correct | split | rebuild event; post-run fix event | Diff-derived owner attribution and later live trigger proof are separate; the selected owner label is advisory when it disagrees with the diff. | current angles 9, 13 |
| X7 | unobservable | procedure-only | X1 plus operator closing note | Terminal and budget facts are deterministic. Goal status, fruitful learning and “safe work remains” are operator judgements. | current angle 13 |
