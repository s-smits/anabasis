 ...
 *
 *  correctnessModelHash stays the byte identity of the whole package. Candidate identity and snapshot
 *  integrity need that: a reference-only repair is a new candidate even when it scores nothing
 *  differently. This hash answers the narrower question every task-only reading asks.
 *
 *  The walk is static, and static is the whole closure: the bundle refuses builtins and every runtime
 *  loader (`import()`, `Bun`, `require`, `eval`; `assertGeneratedSourceLoaders`), and the confined
 *  child reads only the bundle's bytes. A module's name decides nothing: a test the evaluator imports
 *  is scoring. The walk errs towards movement. A type-only import is erased and skipped; an unused,
 *  side-effect or re-export import keeps its file inside, so the reading can over-report a change but
 *  never hide an executed byte.
 *
 *  Bun also compiles each module under the nearest tsconfig.json or jsconfig.json, following its
 *  `extends`, and the nearest package.json: `useDefineForClassFields`, `experimentalDecorators` or a
 *  `.js` file's `type` change the bundle while every walked byte stays put. A package carrying any
 *  such file is therefore read whole. A file above the package is outside this hash and
