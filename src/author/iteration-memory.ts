/**
 * Cross-iteration Builder memory: what this campaign already tried and lost, delivered through
 * the ONE channel a repair pass already carries.
 *
 * The controller knew before the model did. Each evidence hashes its failure as `findingsHash`,
 * but the session that produced a repeat was never told it was repeating: a restarted session receives
 * the last iteration's findings for its owner and nothing else, so an A→B→A oscillation reads
 * to it as a first attempt (campaigns/bridge-truss tripped the generated-identity owner guard in
 * iterations 01 AND 03; the brief session records the symptom and the single-iteration carry
 * that does not reach across the gap).
 *
 * Code owns the fact and the model owns the response: this owner states what happened and never
 * what to author instead.
 *
 * The isolation. Memory is built from structural evidence fields — ordinals, outcomes, the stage a
 * build died at, session attempt counts — plus the (code, path) pairs an owner has already
 * produced, each one through projectFindingForAuthor, which fails closed on anything a
 * controller validator did not write. `CampaignFeedback.claim` and `.evidence` name protected
 * host evidence (the F2 census, verifier stdout) and never enter. Finding DETAIL never enters
 * either: an identifier and its path are enough to say "you already produced this".
 */
 ...
 * and feedback rows `refusalsOf` reads findings out of.
 *
 * `parseJsonAs` casts rather than checks, and the ordinal was the only field anything looked at,
 * so a record left short by an interrupted write reached
 * `Object.entries(undefined)` and threw past the catch below, taking the whole opening advisory
 * with it over one unreadable pass. The catch already says what an unreadable pass is: absent
 * memory. This says the same about one that parses and cannot be summarised.
 */
...
function readEpoch(dir: string): IterationEvidence[] {
  ...
  for (const name of listIterationDirs(dir)) {
    try {
      ...
      // A missing or half-written evidence is absent memory; an unreadable pass is never guessed.
    }
  }
 ...
 * reader bound to its own epoch never delivered. A damaged epoch record, which the controller
 * refuses, leaves this epoch reading only itself.
 */
 ...
 *  what it proposed. A recorded proposal is about 2,000 bytes with its digest; the gap and change
 *  say what was tried, the target what was expected, and the admitted scope what the bytes did. */
 ...
 * completed iteration yet, so a first build's prompt carries no memory section. Both the timeline
 * and the refusal history span the campaign.
 */
export function iterationMemoryFindings(campaignDir: string): ContractFinding[] {
  ...
  // Newest pass first, so the sort's stable ties resolve towards the pass that just ran; each older
  // label is prepended, which keeps every entry's own pass list chronological as it renders.
  ...
  // one-offs from the first, and the line rendered as if nothing were missing. The repeat count was
  // already in the map's value, unread. It is the reading the two other bounded lists beside this
  // one take — `standingIssues` sorts by rate before its cut, the advice packet's issue block by
  // count — and the same defect found in that packet's finding block on 2026-09-18. A label several
  // passes carry is a habit; one pass's label is an incident, and the habit is what this line means.
