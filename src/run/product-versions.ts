...
export function readProductVersion(repoRoot: string, slug: string, id: string): string {
  ...
  // A retained version links its tool tree into an epoch workspace, and those bytes sit outside the
  // fingerprint by design: about 250 MB per epoch, five epochs in one recorded campaign here. An
  // operator reclaiming that space leaves the link unresolvable, and the equality below then refused
  // every retained version of the campaign — 105 of the 109 retained on this machine — which stops a
  // continuation before its opening exists, so no terminal records why. The product's own bytes are
  // untouched and verifyTree above proves it. A tree that resolves somewhere else is still a changed
  // reference and still refused; a tree that resolves nowhere is disclosed as null, and every reader
  // of it already resolves tools on the host PATH and reports the ones it cannot find.
