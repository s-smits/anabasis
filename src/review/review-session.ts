 ...
 *
 * Operator names say `review`, while the Main Judge census names — evidence, pin fields and the
 * verdict schema — say `judge`, because they describe that one caller and appear in recorded runs;
 * renaming them would make an older record unreadable without making a current one clearer. Every
 * transport records a verdict through the one schema tool (judge-drivers.ts), so no transport gets
 * its own parsing of what a verdict is.
 *
 * Two judgeDeAnchoring settings correspond to construction here rather than to a check that
 * enforces them:
 ...
 *  silent broker otherwise sits until the session's generic one-hour ceiling and reports nothing
 *  about which turn died, and half an hour converts it into a typed turn timeout first. The Judge
