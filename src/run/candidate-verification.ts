 ...
 *  the Judge never does. Previously this check read only Judge findings,
 *  leaving a host-reported unbound verdict recorded but unused: the next decision read older
 *  admission, found no blocker and continued climbing. Inspect every admitted row's severity
 *  so that blocking host feedback also controls publication of a held candidate's packet.
 *  Advisory rows alone do not enable publication. */
...
export async function settleCandidateEvaluation(input: PostBuildInput): Promise<CandidateEvaluation> {
  ...
  // A null claim states an environment-blocked battery (live-run-08: dead provider); its recorded
  ...
  // most needs its census and admission read is the one that failed everywhere (2026-08-10, the
