...
/** Resolve the spec's tool, run it through the wall and unconfined, and print one receipt line. */
async function probe(specJson: string): Promise<void> {
  ...
  const spec = JSON.parse(specJson) as ProbeSpec;
...
if (import.meta.main) {
  await runCommand(
    { name: "run-tool", usage: "bun run-tool.mts '<spec-json>'", positionals: 1 },
    ({ positionals: [specJson = ""] }) => probe(specJson),
  );
}
