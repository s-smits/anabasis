...
function target(url: string, address: string): PublicHttpsTarget {
...
describe("public source acquisition through a checked address", () => {
  ...
  // The pin used to be a lookup hook handed to `node:https`, which had to answer two shapes because
  // Node's Happy Eyeballs path asks for every address and rejects a single-address answer. The
  // request now names the vetted address directly, so there is no second resolution to disagree
  // with and no shape to get wrong.
  it("sends the request to the vetted address and keeps the operator's path", () => {
    expect(pinnedRequestUrl(target("https://sources.example/a/archive.tar.gz?v=2", "8.8.8.8"))).toBe(
      "https://8.8.8.8/a/archive.tar.gz?v=2",
    );
  });

  it("brackets an IPv6 address so it is a legal URL authority", () => {
    expect(pinnedRequestUrl(target("https://sources.example/x", "2606:4700::1111"))).toBe(
      "https://[2606:4700::1111]/x",
    );
  });

  it("keeps the original hostname for HTTP and TLS while connecting to the vetted address", async () => {
    // SAFETY: this test double returns the complete DNSLookup fields consumed by the production lookup.
    Bun.dns.lookup = async () => [{ address: "8.8.8.8", family: 4 as const, ttl: 60 }];
    ...
    try {
      expect(requests).toEqual([
        {
          url: "https://8.8.8.8/archive.tar.gz?v=2",
  ...
  it("sends an IP literal through the resolver and refuses a short loopback spelling", async () => {
    ...
    expect(pinnedRequestUrl(resolved)).toBe("https://8.8.8.8/archive.tar.gz");
