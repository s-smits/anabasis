...
export const POLICY = {
  climb: {
     ...
     *  consumer receives it. The Builder's battery contract and src/run/battery-sizing.ts read
     *  this constant directly until 2026-09-21, so a manifest override moved the recorded placement
     *  while the Builder's prompt kept quoting these counts and the sizing gate kept holding the
     *  code-owned ceiling: three owners of one number, disagreeing only when a run overrode it. They
     *  now take the band as a parameter and the controller passes `climbThresholds(...).band`; the
     *  default they declare is this row, for a caller with no manifest. */
    band: CLIMB_BAND,
  ...
  loop: {
     ...
     *  to end the campaign, and the recomputed next decision often permits a retry; the count and
     *  its recorded history live at src/run/full-run-round.ts. */
    buildFailedRounds: 3,
     ...
     *  anything starts a new key. The run that motivated it is named at
     ...
     *  changes the tree while the unsuccessful tool run does not, so the no-op strike counter sees a
     *  different tree every time; the run that showed this is named at src/author/tool-non-result.ts.
     *  Three matches `noopSubmitStrikes`: report the defect, allow the repair, end the repetition. */
