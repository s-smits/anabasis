 ...
 *  submit enforces, so a generated tool can no longer narrow numbers and strings or widen object
 *  and file-map keys behind a second, hand-written schema. */
export function publicArtifactWriterParameters(schema: PublicArtifactSchema): TSchema {
  ...
  // the same schema purely because its wire representation is canonical. The child consumes the
  // root the parent already authenticated, and the parent keeps the original hash for binding
  // evidence.
