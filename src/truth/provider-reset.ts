 ...
 * just handed it, because `PROVIDER_ALLOWANCE` reads both as spent. The clause is the provider's
 * own number, so `awaitTurnRetry` sleeps on it rather than guessing a backoff that cannot reach it.
 *
 ...
 *  the whole list, so a session limit naming 12pm spoke for a monthly spend limit beside it: that
 *  one names no clock and no wait clears it, and the turn slept until noon to be refused again.
 *  Where several allowances stand, work resumes when the last of them lifts, not the first. */
