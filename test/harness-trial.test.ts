...
describe("what a rehearsal hands the round plan", () => {
  ...
  it("records the verdict and effort, returns the plan's advice, and offers only a passing trace", async () => {
    ...
    // Both solves ran on the same bytes, and the row names the public task the result names.
    expect(asRecord(passing.task)?.publicTaskDigest).toBe(rows[0]?.bytes.publicTaskDigest ?? "no row");
    expect(rows[1]?.bytes).toEqual(rows[0]?.bytes);
