 ...
 *  crosses only with a probe behind it. Five 2026-09-19 to 22 runs showed 33 advisory rows in tool
 *  results, repeats of one another and promised "for the next round", which no reader carried;
 *  none changed what the Builder did. The post-battery review weighs the product again. */
...
/** Carry an authoring review's disputes onto the issue register the next build reads. The measured path
 *  does this inside its own publish step; this one used to write the review file and stop, so a
 *  dispute the reviewer was invited to make reached nothing that reads it, and the next build was
 *  still told to rebuild the agent around an issue a review that had read the tree called an
 *  evaluation defect. Campaign -10 disputed two of the three issues its `rebuild-advice-latest.json`
 *  still carries as `"dispute": null`. The register is re-read here rather than reused from the turn
 *  above: a measured battery may have published between the two moments. */
