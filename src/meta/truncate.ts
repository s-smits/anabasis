// Vendored from pi-mono packages/agent/src/harness/utils/truncate.ts at 086c32e74 (main, 2026-08-15).
// Upstream fixed the trailing-newline tail truncation that returned zero bytes for a >maxBytes last
// line ending in "\n". test/pi-truncate.test.ts is the matching upstream test, copied at the same
// revision.
    ...
    if (code >= 0xd800 && code <= 0xdbff) {
      if (i + 1 < content.length) {
        ...
        if (next >= 0xdc00 && next <= 0xdfff) {
          // (?? ""): in-bounds by the loop guard; this repository compiles with noUncheckedIndexedAccess.
