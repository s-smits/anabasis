 ...
 * resets the counter like any other delivery. One whose claim was refused counts: campaign
 ...
 * only when the Builder's own turn was refused. Rule 15: do not remeasure the same dead provider
...
export function nextBlockedRounds(prev: number, result: IterationResult): number {
  ...
  // normally proves it completed turns, with one exception: the provider-stop rule (five
  // consecutive provider non-results end scheduling) records the battery `provider-stopped`, and
  // the claim it then writes is a refusal for that same dead provider, not evidence that the
  // provider worked.
  const delivered =
 ...
 *  (the difficulty evidence) and no admitted feedback. The entry decision is the reliable carrier of
 *  the reason — `nextDecision` is recomputed solely for failed builds and unadopted candidates and
 *  stays null on a reused round — but the entry alone counted the round that finally produced
 *  evidence, and at the third such round `loopTerminal` ended the run before the next selection
 *  could read what it had just been given. Any round of another shape resets the count. The default
 *  round cap was this spin's accidental bound until its removal on 2026-08-19. */
