...
10. **Bracket a witnessed capability boundary.** Every admitted task has a verifier-accepted
    ...
    it does not separate passes from fails. For the latest battery's passing cases it also states
    each declared public comparison's shipped value, limit and signed slack
    (`src/run/passing-slack.ts`, through `publishedMargins` and `readMargins`), and says unknown
    rather than zero where no complete `numericBoundaries` row is declared. Useful adopted work is
    retained.
...
12. **Evaluate the requested artifact, not decorative output.** Every artifact-schema root must be
    ...
    and a family is refused when one sibling's deliverable passes every other task and two siblings
    also pass each other's (`familyBindingFindings`, `src/truth/family-binding.ts`), so a ladder that
    tightens a published limit clears. Numeric boundaries get a task sitting on the value, and a
    root no check reads is refused before measurement.
...
14. **Keep model-visible text small and single-owned.** Stable domain and safety framing belongs in
    ...
    its bytes check against the evidence log), and the `--context` files. Until 2026-09-23 it read the `--context` files alone, and 160
    ...
    and any typed non-result, under six rehearsals per round, graded under the harness's own
    `check_seconds` and `tool_run_seconds` as a measured battery is. Each rehearsal costs one measured case and writes its solve evidence under
