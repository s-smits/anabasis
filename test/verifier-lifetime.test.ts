...
describe("durable verifier ownership", () => {
  ...
  it("reports active intents to terminal closure and prevents new admission", async () => {
    ...
    // One sentence covered every stop there was, and it was the sentence written for the first: the
    // 2026-09-18 investigation read "restore the host" against 1714 receipts that had every one
    // settled, because the throw was this one — a begin after close. `solvability.ts` carries the
    // message into an environment-owned finding, so the wrong sentence routes as evidence.
