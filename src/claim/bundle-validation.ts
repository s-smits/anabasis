...
/**
 * Detect possible answer keys by filename. A name cannot prove what a file contains; the public
 * projection and sandbox restrict the actual data available to the agent. Every leaked answer key
 * observed so far had a recognisable filename, so this inexpensive check catches known mistakes.
 */
...
/** Collect module specifiers from the syntax tree. The previous regular-expression check
 *  matched the words from/import/require inside unrelated string and array
 *  literals. `["from", "fromJoint"]` produced the false specifier ", " and caused a valid
 *  bundle to be refused (campaigns/bridge-truss 02, 2026-07-26). The syntax-tree walk reads
 *  actual dependencies: import/export declarations, `import =` external references,
 *  dynamic import(), and require() calls; `loads` keeps each literal operand node. */
...
export function validateAgentBundle(agentDir: string, opts?: { allow?: string[] }): BundleValidationResult {
  ...
  try {
    ...
    // A symlink the walker skipped would be unhashed and unscanned. Validation would then approve
    // a bundle whose runtime import graph it never saw (handover 2026-07-11).
    return {
      ok: false,
