 ...
 * (source 5515372d) and run55-sol-0903 (source 66536c5b) both settled "Your access token could
 * not be refreshed because you have since logged out or signed in to another account", and
 * truss-run12-sol-0903 settled the per-turn settle cap while the host had no internet for an
 * hour. Each failure was transient: the campaign, the session workspace and the candidate all
 * survived it, and the only thing that ended was the run.
 *
 * Retry failed/aborted no-output turns, except disabled accounts and allowances that name no
 * clock. The original typed failure still owns environment-blocked classification. Generic 429
 * responses, overload and token-refresh errors remain eligible for these retries.
 *
 ...
 * (Europe/Amsterdam), spending no wait at all: the allowance matcher read a clock-bounded
 * session limit as exhaustion. Seventeen minutes of ladder could not have reached either clock,
 * so the wait has to be the provider's own number. The three attempts still bound it.
 *
 * A retry is a paid attempt: it reserves its own provider turn through the same two gates the
 ...
 *  waits grow because the observed causes clear on different clocks — a re-login lands in
 *  minutes, a home network outage took closer to an hour. */
...
export async function awaitTurnRetry(
  ...
  // Ask the paid admissions first: a retry that the budget or a controller stop would refuse must
  // not spend the wait before finding out.
  ...
  // an abort at 200 ms returns at 201 ms and exits the process at 8002 ms; the same race over a
  // timer the abort clears exits at 201 ms. A run stopped during a wait until a provider-named
  // reset would keep its process alive to that reset with terminal.json already written.
