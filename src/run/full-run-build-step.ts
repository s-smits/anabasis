 ...
 * from unchanged tasks. Truss -14 i02 recorded that subset's 5/5; i03 lost it and wrote 24/25.
 * Recover only byte-matched public authoring metadata; no score or verdict is carried forward. */
 ...
 *  invocations. `unchangedCandidateSubmissions` is the durable per-commit tally counting this record,
 *  so reaching the ceiling adds the exact `authoring-stalled` literal the terminal reads
...
function settleBuildOutcome(
  ...
  if (buildAdmissible) {
    ...
    // when the build had failed. The `adopt` row has the same shape: until now it said only
    // "already adopted", so a reader tallying adoptions counted the rounds that adopted nothing.
 ...
 *  "Build step started (rebuild)": the campaign rethrows a BuildAgentTurnNonResult before any row. */
