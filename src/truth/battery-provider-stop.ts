 ...
 * case at once rather than one of them, so a single outage can take most of a battery with it and
 * paying for the rest buys nothing but more of the same non-result. The Judge census already stops
 * its paid calls after five consecutive provider errors, and the battery mirrors that rule: after
 * BATTERY_PROVIDER_STOP_CONSECUTIVE consecutive provider-kind solver non-results, no further case
 * is scheduled.
 *
