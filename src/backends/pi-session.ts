 ...
 *  the transcript's system messages, so a later round replaces this section by name instead of
 *  appending a second framing beside the first. */
...
/**
 * A short, whitespace-collapsed preview of a tool result's text, for trace logging only. The clip
 * keeps both ends of an overlong preview, with a marker between: a failing command reports its
 * error last — "exit code 1" after pages of build output — so a head-only slice shows exactly the
 ...
 * holds the answer (an accepted or finally refused Builder submit, a Built submit, a Judge
 * verdict). The turn's other calls still return, and no further provider request follows. Pi on
 * its own ends only a batch whose every call terminates, so a submit sharing an assistant message
 * with another call paid for one more request. On the Claude CLI one query spans the whole prompt
 * and still waits for this turn's tool results, which nothing would deliver, so ending also closes
 * that query; before, it waited until the process went.
 */
...
export class PiPromptRecord {
  ...
  /** The last assistant message the provider answered: it named the served model, or at least
   *  gave the response its id. The Codex route never names a model (`UNATTESTING_PROVIDER` in
   *  runtime-model-identity.ts), so the id alone is what it attests. An ending nothing answered
   *  names neither, so a stopped turn attests what answered before it. */
  served: AssistantMessage | undefined;
  private compactionEstimate = 0;
  /** The bridge observes a builtin call twice — on the streamed block and on the assembled message —
   *  and one search must enter the trace once. */
  ...
  /** A CLI-owned builtin answers inside the CLI's own loop, so it never reaches pi's tool events and
   *  would otherwise leave the trace claiming no such tool was used. The host sees the call, not its
   *  result, so the pair is recorded as a started-and-ended call without error. */
