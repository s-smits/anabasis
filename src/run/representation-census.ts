/**
 * Check the reference witnesses already produced by F2 for two representation problems.
 * F2 checks solvability; these comparisons detect copying and arbitrary absence spellings.
 *
 * live-run-06 is why. It passed F2 25/25 and then measured 0/25 with every submit accepted
 * and the accept-controls passing 3/3. Its retained traces show the cause of those failures:
 * the cause: the reference answer writes `accessionCode: "n/a"` for records without a local code, the
 * check demands that exact spelling, and the agent wrote "" instead — 95 times, in 25 of 25 cases.
 * A whole measured campaign turned on how one absence is spelled.
 *
 * Findings describe the Builder's artifact schema and public task projection, without exposing
 * protected verifier evidence: no task id, no check id,
 * no hidden expectation, no verifier text. Both are pure comparisons over witnesses already in hand,
 * so neither costs an execution.
 *
 * The two checks were calibrated by replaying this module over seven recorded F2 results
 * available at the time. Six paired with a task file containing their exact task
 * ids; the seventh has none on disk and is left out instead of guessed:
 *
 *                        transcribes (blocking)     absence sentinel (blocking)   measured
 ...
 *
 * `transcribes` was initially advisory and became blocking after run 67. It occurred in both
 * a 0/25 bundle and a 25/25 bundle, so it did not predict failure. Run 67 demonstrated why a
 * high score could still reflect the problem: the finding identified a schedule root copied
 * from publicInput.behaviours on 125/125 reference witnesses across five fingerprinted task
 * sets. Those iterations still passed fingerprinting, and six batteries then
 * measured 150/150 passes over levels L0-L4 — a battery with no limit, because its verified root
 * could be filled by retyping the task. A high pass rate does not resolve a copying finding,
 * since copying may itself make the tasks easy. `absence sentinel` flagged only the 0/25 bundle.
 * Three of the six calibration rows had measured batteries, so the measured sample is small.
 * The blocking policy follows these recorded failures and the cost of spending a campaign
 * measuring them. A false positive still requires the Builder to revise the representation.
 *
 * A domain that needs "none" as an answer declares it in that field's allowedValues:
 * run 52 refused a candidate for writing a value its own public schema listed as one member of a
 * closed set, so the absence rule now exempts a value the schema names at that exact path
 * (`declaredClosedValue` below). The earlier policy documented vocabulary exceptions in a
 * deviation row with an expiresWhen. Nothing below reads such a row, so it supplies no runtime
 * override; the declared closed values are the exemption this code actually enforces.
 */
...
/** Answers that state "this does not apply" instead of deciding something. Kept short and
 *  unambiguous on purpose: "na" and "nil" are omitted because they are real values in some
 *  domains (sodium, a null literal) and a census that blocks may not guess. */
 ...
 *  nothing, so a path the schema does not reach is simply unexempted. */
 ...
 *  why: a field whose allowedValues are ["none", "flat", "reactive"] compiles to a closed node, and
 *  the census refused the candidate for writing the declared "none". The exemption is deliberately
 *  narrow. Every node describing the path must be closed: a closed set beside a plain string, or
 *  beside null, still admits an undeclared spelling of the same absence, which is the defect this
 *  rule guards. An absent schema exempts nothing. */
 ...
 *  `[]`. One sibling is enough to record the path: a check that evaluates the spelling still fires
 *  on the rows that use it, and siblings deciding something do not clear those rows. The public
 *  schema descends alongside the value so a declared closed state is recognised at its own path. */
 ...
 *  schedule was the copied rows in another order, and one reordered witness would likewise
 *  silence the every-witness aggregate below. So copy detection ignores order at the compared
 *  collection itself — its rows sorted by their canonical JSON — and nowhere deeper: a nested
 *  array is an ordered value (a directed edge, a coordinate pair, a route), and an artifact that
 *  reverses each one has transformed the data, not retyped it. canonicalJson stays the only
 *  serializer. The known cost is a domain whose requested answer is a pure reordering of a public
 *  list: its reference root now matches as a copy, and the finding's stated options (fold the
...
export function observe(witness: Witness, schema?: PublicArtifactSchema | null): Observation[] {
  ...
  // each keyed once, so neither key order nor row order is read as a difference and no side is
  // walked twice.
...
/**
 * Report a property only when it holds on every witness: one task whose data
 * happens to need an absence, or whose selection happens to be the whole catalog, is that task's
 * shape; all of them is the schema's.
 */
export function censusRepresentation(witnesses: readonly Witness[], schema?: PublicArtifactSchema | null) {
  ...
  for (const witness of witnesses) {
    for (const observation of observe(witness, schema)) {
      ...
      // collection applies, and the detail sentence naming a single source would be false.
      const key =
...
/** The kinds that refuse adoption. Split here so the gate never re-decides severity by hand. */
export const BLOCKING_CODES = new Set([CODE["absence-sentinel"], CODE.transcribes]);

/**
 * Detect a reference root that stays constant across different public inputs. Run 12 prompted this:
 * its reference solve read publicInput fields absent from the authored tasks (busDevices where the
 * tasks carry requestedItems), so `solve` derived the same allocationPlan — one byte-identical value —
 * for 25 tasks whose public inputs all differ, and F2 refused 25/25. The aggregate count alone
 * crossed to the Builder, and iteration 2 failed identically: the count says the solve is wrong
 * everywhere, while this observation identifies a concrete pattern the model can investigate.
 *
 * Both compared sides are the Builder's own facts — its solve's outputs, its authored public
 * inputs — so the finding can describe their relationship. It includes no task id,
 * check id, verifier output or hidden expectation.
 *
 * The empty-proves-nothing rule (`bulky`) inverts here on purpose: for copying, empty matching
 * empty is no evidence; for responsiveness, a derived root that stays empty while every input
 * differs is still relevant: run 12's constant value was the empty plan. A root that
 * is legitimately constant is still a true observation with its denominators stated; the model
 * assesses whether that constancy is intended. These findings carry no severity of their own;
 * they accompany an already blocking census result as diagnosis, leaving BLOCKING_CODES unchanged.
 */
