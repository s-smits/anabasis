 ...
 *  counter. What is left is whether it answers the request it names, and whether the checkpoint
 *  it carries may replace the one the controller holds. Deciding that here, over three values,
 *  leaves the caller its pending map, its timers and its failure latch, and names each outcome
...
/** What the controller keeps from a frame it did not refuse. The checkpoint is the worker's
 *  state, not the call's: the checks below have already proved it against the held one, and a
 *  failed call still leaves the draft the next call continues from. `taskAccess` is what the
 *  frame reported, or nothing when its kind carries no such field; the caller keeps what it holds
 *  in that case. */
...
/** The three things that can happen to a reply. `fail-call` is the only one the session
 *  survives: the tool the solver asked for said no, which is that call's business.
 *
 *  Both surviving verdicts carry the retained state, so the caller has no branch that can drop
 *  it. Until 2026-09-20 the failing branch returned before the assignment, and a turn whose tool
 *  call failed lost the checkpoint the worker had just recorded — the liveness evidence a long
 *  turn is read through (AGENTS.md working rule 6). */
type ResultVerdict =
