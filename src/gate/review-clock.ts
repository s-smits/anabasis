 ...
 * review, that immutable snapshot; after the backstop interval, a snapshot of the workspace frozen
 * then. Both are consulted at the completion of a host tool call, never inside one, and the review
 * runs beside the session (`src/run/authoring-review.ts`).
 *
...
/** What the next completed tool call owes the reviewer: a validated product snapshot, a freeze of
 *  the workspace, or nothing yet. */
type ReviewDue = { kind: "repair"; fingerprint: Fingerprint } | { kind: "backstop" } | null;

export class AuthoringReviewClock {
  /** The last clear `correctness_check`'s product, consumed by the next completed tool call that
   *  finds no review running; a later clear check replaces it. */
  ...
  /** This product has now been read, by a repair review or by a backstop review of the same frozen
   *  bytes. */
