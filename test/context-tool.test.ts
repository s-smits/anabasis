 ...
 *  records beside it, filled from `secret`. */
function recordedTree(secret: string): string {
  ...
  for (const taskId of ["t0", "t1", "t2", "t3"]) {
    ...
    evidence.write(`cases/${taskId}/public-task.json`, {
      ...
      publicTask: { taskId, family: "frame", publicInput: { span: 3 } },
    });
    if (taskId !== "t3") {
      evidence.write(`cases/${taskId}/artifact.json`, { design: { members: [`${taskId}-member`] } });
    }
...
describe("the traces source", () => {
  it("offers each passing case's solve and recorded artifact, and never a failing one or a non-result", async () => {
    ...
    const [head, , ...body] = artifact.split("\n");
    ...
    expect(JSON.parse(body.join("\n"))).toEqual({
      runId: "r1",
      publicTask: { taskId: "t0", family: "frame", publicInput: { span: 3 } },
      submittedArtifact: { design: { members: ["t0-member"] } },
    });
  });
