 ...
 * session declaring its own census width departs from, and it declares that once rather than per call. Callers in one run
...
export function reviewConcurrency(env: Record<string, string | undefined> = Bun.env): number {
  const raw = env.ANA_REVIEW_CONCURRENCY;
  if (raw === undefined || raw === "") return JUDGE_MAX_CONCURRENCY;
  if (!/^[1-9]\d*$/.test(raw)) {
    throw new Error(`ANA_REVIEW_CONCURRENCY must be a positive integer, got "${raw}"`);
  }
  return Number(raw);
}
